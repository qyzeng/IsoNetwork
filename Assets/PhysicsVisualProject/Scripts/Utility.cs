...
public static class Utility
{
	...
	public static Transform GetHeadNode (this GameObject target)
	{
		Transform headNode = target.transform.Find ("EHead");
		if (headNode == null) {
			Transform center = target.transform.GetCenterNode ();
			Bounds calcBounds = new Bounds (center.position, Vector3.zero);
			Renderer[] renders = target.GetComponentsInChildren<Renderer> ();
			...
			headNode = new GameObject ("EHead").transform;
			headNode.transform.position = new Vector3 (calcBounds.center.x, calcBounds.max.y-0.2f, calcBounds.center.z)+target.transform.forward*0.1f;
			//headNode.transform.position = center.position + 0.8f* Vector3.up;
			headNode.transform.parent = target.transform;
		}
		return headNode;
	}
